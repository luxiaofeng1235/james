...
if [ ! -n "$result" ]; then
    ...
    while IFS=$'\t' read -r column1 column2 column3 column4; do
        ...
        shell_cmd="cd /www/wwwroot/work_project/novelProject/biquge/ && nohup /www/server/php/72/bin/php synBiqugeBook.php $column1 >> run_biquge.out 2>&1 &"
        echo $shell_cmd | bash;
    done <<< "$result"
fi
